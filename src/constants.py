...
JD_PATH = "../jd_data/*"
# Defaults to the synthetic dataset since resume_data/ (real resumes, PII) is gitignored and
# won't exist on a fresh clone. Point this at "../resume_data/*/*" instead for local testing
# against real resumes.
RESUME_PATH = "../resume_data_synthetic/*/*"
...
You may also be given a match score: a percentage (0-100) representing the embedding similarity between the resume
and job description. If asked about this score (e.g. why it isn't higher, or what it means), explain it by pointing
to specific gaps or alignments between the resume and job description content - never invent a reason unrelated to
the actual content provided.
Strictly ONLY answer the question if it is relevant to the resume, job description, or match score provided
otherwise reply with 'Please ask a relevant question'. Do not answer general knowledge questions.
"""
